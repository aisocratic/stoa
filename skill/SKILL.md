---
name: ai-socratic-design
description: The AI Socratic design system — layout skeleton, golden-ratio type scale, color tokens, and the reusable component/form library. Use WHENEVER creating or editing a webpage, component, form, dialog, card, hero, section, or any UI in this repo (public site or admin), so new UI reuses the existing system instead of inventing new styles or components. Triggers: "new page", "add a section", "build a form", "create a component", "style this", landing pages, admin pages, CTAs, cards, tables.
---

# AI Socratic Design

The AI Socratic design system, shipped as `@aisocratic/design`. It is what aisocratic.org,
Agora and Atlas are built on. **Reuse before you create**: search
`@aisocratic/design` and the app's own `components/ui/` before writing any new component,
...
3. **Semantic color tokens only** — `bg-background`, `text-foreground`,
   ...
   avoid raw palette colors (`bg-slate-100`, inline hex) entirely. The palette
   (`--oat-*`, `--ink-*`, `--amber-600` …) is for re-theming a role, never for a class.
...
4. **Page width = `page-shell`** (the custom utility ≈ `max-w-6xl mx-auto px-4 md:px-6`).
   **Page skeleton = `Section` / `SectionHeading` / `RuledHeading` / `PageHero`** from
   `@aisocratic/design` (`Section`). First section of a page gets `lead` (clears the fixed
...
5. **Two radii:** `rounded-md` (10px) for controls, `rounded-xl` (16px) for surfaces,
   `rounded-full` for pills. Other names alias to these; write the canonical one.
6. **Buttons:** `Button` (`@aisocratic/design` (`Button`)) for actions — use its `loading`
...
7. **Form fields:** `TextField`, `SelectField`, `ToggleField` from `@aisocratic/design`
   (grouped in `FormSection`). Fields carry their own label/description/error chrome.
   Never compose raw `Label`+`Input`+error text inline. Admin lists are `DataTable`;
   sign in / sign up is `AuthPanel` / `AuthScreen`.
8. **Toasts** via `sonner` (`Toaster` from `@aisocratic/design/components/sonner`).
9. **`cn()` from `lib/utils.ts`** for all class merging — it's configured to understand
   ...
   `text-body` etc.).
10. **Cards:** `Card`/`cardSurface` from `@aisocratic/design` (`Card`) for boxed surfaces;
    editorial feeds use the open (border-less) `StoryTile` pattern. Icons: `lucide-react`
    only.

## Reuse order

`@aisocratic/design` primitive → `@aisocratic/design` composed pattern (`SectionHeading`,
