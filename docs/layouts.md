...
Canonical skeleton lives in `@aisocratic/design` (`Section`). It formalizes what older
...
`SiteHeader` and `components/mobile-menu` from the package ARE this chrome —
render them, pass `links` / `items` from the app's navigation config and
`linkComponent={Link}`. The numbers below are what they encode; do not
re-derive them — they are load-bearing for every page's `lead` clearance.
...
- Desktop links: `text-nav` (body size, uppercase, 0.08em, the body face);
  active `text-foreground font-semibold`, idle `text-foreground/50
hover:text-foreground/80` — `navLinkClass(active)`.
- Mobile top-level links: `text-title uppercase tracking-[0.08em] py-2`; active
...
**Admin sidebar (`AdminShell`):** separate chrome.
...
- **Boxed card** — `BlogPostCard` recipe:
  `flex flex-col border rounded-xl overflow-hidden bg-card transition-colors group hover:border-primary/40`.
  For generic surfaces use `Card`/`cardSurface` from `@aisocratic/design` (`Card`).
- **Empty states** — `EmptyState` from `@aisocratic/design` (`EmptyState`)
...
- Primary: `inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-md bg-primary text-primary-foreground text-body font-medium hover:bg-primary/90 transition-colors`
- Secondary/outline: `inline-flex items-center justify-center gap-2 px-5 py-2.5 border border-border rounded-md text-body text-foreground/80 hover:text-foreground hover:border-foreground/40 transition-colors`
...
- Newsletter prompts: `NewsletterPrompt` / `components/newsletter-form.tsx` (the
  ...
  normal forms).
- Stats: `MetricCard` in a `grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8`.
